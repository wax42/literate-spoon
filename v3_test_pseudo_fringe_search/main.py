...
class Taquin():
	def __init__(self, heuristique, dim, goal, map):
		...
		self.max_depht = 1000
		self.incr_depht = self.max_depht * 100
		...
		for i in neightbours:
			...
			if pos_x >= 0 and pos_y >= 0 and pos_x < taquin.dim and pos_y < taquin.dim:
				...
				if (newnode_map_str not in hash):
					...
					# ajout d ewl update de la depht ici
					newnode.depth = data.depht + 1
					if ((newnode.depth % taquin.max_depht) == 0):
						newnode.g += taquin.incr_depht
